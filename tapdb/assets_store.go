...
func (a *AssetStore) LogPendingParcel(ctx context.Context,
	...
	return a.db.ExecTx(ctx, &writeTxOpts, func(q ActiveAssetsStore) error {
		...
		if len(existingTransfers) > 0 {
			// Breach recovery can re-deliver a NotifyBroadcast for the
			// same pre-anchored sweep/import tx. Treat the transfer shell
			// as idempotent by anchor txid so we don't strand duplicate
			// pending rows for the same on-chain spend.
...
func (a *AssetStore) LogAnchorTxConfirm(ctx context.Context,
	...
	err := a.db.ExecTx(ctx, &writeTxOpts, func(q ActiveAssetsStore) error {
		...
		for idx := range inputs {
			...
			// need a template asset row to create the successor asset and
			// keep the porter moving instead of leaving the transfer in a
			// permanent pending state even though the BTC spend confirmed.
			templateAssets, queryErr := q.QueryAssets(ctx, QueryAssetFilters{
			...
			if queryErr != nil {
				return fmt.Errorf("unable to find template asset: %w",
					queryErr)
			}
			if len(templateAssets) == 0 {
				return fmt.Errorf("unable to find template asset for "+
					"missing spent input, script_key=%v, asset_id=%v, "+
					"anchor_point=%v", spew.Sdump(inputs[idx].ScriptKey),
					spew.Sdump(inputs[idx].AssetID),
					spew.Sdump(inputs[idx].AnchorPoint))
			}
